...
const _string = Codec.make<string, string>(AST.StringKind)
export const string = <S extends string>(): Codec<S, string> => _string as any

export const boolean = Codec.make<boolean, boolean>(AST.BooleanKind)
export const forNull = Codec.make<null, null>(AST.NullKind)
export const array = <C extends Codec<any, any>>(codec: C) =>
    Codec.make<GetInput<C>[], GetOutput<C>[]>(AST.IterableKind(codec.ast))

export const pass = <In>(): Codec<In, In> => Codec.make(AST.PassThrough)

export const forClass = <T extends new (...args: any[]) => any, Out>(_: T, encode: (c: InstanceType<T>) => Out) =>
    Codec.make<InstanceType<T>, Out>(AST.ClassKind(encode))
...
export const transform = <In1, Out1, Out2>(source: Codec<In1, Out1>, f: (source: Out1) => Out2): Codec<In1, Out2> => {
